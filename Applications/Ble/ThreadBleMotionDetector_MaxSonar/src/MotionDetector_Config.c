 ...
 *
 * BLE GATT attribute table for the QPG6200 Thread+BLE MaxSonar Motion Detector.
 *
 * Three GATT services:
 *
 *  1. Battery Service  (0x180F)      - standard, visible in nRF Connect
 *  2. Motion Detection Service       - motion status + distance notifications
 ...
 *  b) Write Thread Network Name characteristic (up to 16 bytes).
 ...
 *  a) Enable notifications on the Motion Status characteristic.
 *  b) Sensor detects object within 2 m -> notification value 0x01.
 *  c) Object moves beyond 2 m         -> notification value 0x00.
 *  d) Subscribe to Distance for raw cm readings.
 */
...
/* Motion Detection Service UUID (128-bit) : M0T10N-0000-1000-8000-00805F9B3400 */
#define MOTION_SERVICE_UUID_128 \
    0x00, 0x34, 0x9B, 0x5F, 0x80, 0x00, 0x00, 0x80, \
    0x00, 0x10, 0x00, 0x00, 0x4E, 0x4F, 0x54, 0x4D

/* Motion Status Characteristic UUID      : M0T10N-0000-1000-8000-00805F9B3401 */
#define MOTION_STATUS_CHAR_UUID_128 \
    0x01, 0x34, 0x9B, 0x5F, 0x80, 0x00, 0x00, 0x80, \
    0x00, 0x10, 0x00, 0x00, 0x4E, 0x4F, 0x54, 0x4D

/* Distance Characteristic UUID           : M0T10N-0000-1000-8000-00805F9B3402 */
...
/* Motion Status characteristic: read + write + notify */
...
static uint8_t        motionStatusValue[1]   = {MOTION_STATE_NONE};
static uint16_t       motionStatusValueLen   = MOTION_STATUS_LEN;
static uint8_t        motionStatusCcc[]      = {UINT16_TO_BYTES(0x0000)};
static const uint16_t motionStatusCccLen     = sizeof(motionStatusCcc);

/* Distance characteristic: read + notify, 2 bytes big-endian cm */
...
static uint16_t       motionDistValueLen     = MOTION_DIST_LEN;
...
static const attsAttr_t Motion_GATT_List[] = {
    ...
    /* Motion Status: read + write + notify */
    { attTypeCharUuid, (uint8_t*)motionStatusCh, (uint16_t*)&motionStatusChLen, sizeof(motionStatusCh), 0, ATTS_PERMIT_READ },
    { &motionStatusCh[BLE_CHARACTERISTIC_VALUE_UUID_OFFSET], motionStatusValue, &motionStatusValueLen, MOTION_STATUS_LEN, ATTS_SET_WRITE_CBACK | ATTS_SET_UUID_128, ATTS_PERMIT_READ | ATTS_PERMIT_WRITE },
    ...
    { &motionDistCh[BLE_CHARACTERISTIC_VALUE_UUID_OFFSET], motionDistValue, &motionDistValueLen, MOTION_DIST_LEN, ATTS_SET_UUID_128, ATTS_PERMIT_READ },
...
static uint8_t        threadChannelValue[1] = {15};
...
/* "QPG MaxSonar Motion" = 19 characters */
static const uint8_t ScanRespFrame[] = {
    0x14,                               /* length: 19 chars + type = 20 */
