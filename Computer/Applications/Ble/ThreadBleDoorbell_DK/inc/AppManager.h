...
/** @file "AppManager.h"
 *
 * Application manager for the QPG6200 Thread+BLE Doorbell DK demo.
 *
 * Responsibilities:
 *   - BLE stack initialisation and event handling
 *   - Thread network commissioning (credentials stored in NVM)
 *   - Digital doorbell button (PB2, GPIO 5) ring event handling
...
public:
    ...
    /* Called by DoorbellManager polling task when digital button state changes */
