...
/** @file "qPinCfg.h"
 *
 * GPIO / Peripheral pin definitions for the QPG6200 Thread+BLE Doorbell DK Analog demo.
 *
 * Board: QPG6200L Development Kit (QPG6200L DK) - standalone, no carrier board required.
 *
 * Analog input (doorbell button):
 *   ANIO1 / GPIO 29  (DK expansion header) - Analog doorbell button
 *     No jumper required - connect button directly to the DK header pin.
 *     ADC threshold press   : voltage > 1.5 V
 *     ADC threshold release : voltage < 0.5 V  (hysteresis)
 *     Typical wiring: 10 kΩ pull-down from GPIO 29 to GND, button connects GPIO 29 to 3.3 V.
 *
...
/* Analog doorbell button on ANIO1 (GPIO 29, DK expansion header).
 * NOTE: GPIO 29 is NOT registered as a digital GPIO - it is configured by
 * DoorbellManager via qDrvGPADC_PinConfigSet() as an analog input.
 * Do NOT add it to QPINCFG_BUTTONS or QPINCFG_UNUSED. */
#define APP_DOORBELL_ANIO_PIN   ANIO1_GPIO_PIN          /* GPIO 29 */
 ...
 * GPIO 29 (ANIO1) is deliberately excluded - it is claimed by the ADC driver.
 * GPIO 28 (ANIO0) is excluded - not used in this application. */
#define QPINCFG_UNUSED                                                                       \
    QPINCFG_GPIO_LIST(PB2_BUTTON_GPIO_PIN, PB4_BUTTON_GPIO_PIN,                             \
                      PB5_BUTTON_GPIO_PIN, SW_BUTTON_GPIO_PIN,                               \
