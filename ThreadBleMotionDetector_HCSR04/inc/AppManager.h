...
/** @file "AppManager.h"
 *
 * Application manager for the QPG6200 Thread+BLE HC-SR04 Motion Detector.
 *
 * Responsibilities:
 *   - BLE stack initialisation and event handling
 *   - Thread network commissioning (credentials stored in NVM)
 *   - HC-SR04 sensor motion event handling
 *   - Thread mesh event handling (forward remote motion events locally)
 *   - LED state machine
 */
...
public:
    ...
    /* Called by SensorManager task when motion state changes */
...
private:
    ...
    /* Report motion state locally (LED + BLE notification + Thread multicast) */
